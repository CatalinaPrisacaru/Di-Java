...
def ColorRange ( db, parent, rec, userData ):
	result = MG_FALSE
	if mgGetCode ( rec ) == fltPolygon:
		outs = mgGetAttList (rec, fltPolyPrimeColor)
		if outs[0]:
			if outs[2] > -1 and outs[2] < 10:
				result = MG_TRUE
	return result

def WalkFromThere (db, parent, rec, map):
	mgSendMessage (MMSG_STATUS, "WalkFromThere called")
	code = mgGetCode (rec)
	if map.has_key(code): map[code] = map[code]+1
	return MG_TRUE

def PrintNodeByCode (db, parent, rec, map):
	mgSendMessage (MMSG_STATUS, "PrintNodeByCode called")
	mgWalk (rec, None, WalkFromThere, map, MWALK_ON)
	return MG_TRUE
...
def main ():
   ...
   # make a simple database hierarchy 
	...
	list = mgFind (db, ColorRange, None, MWALK_ON)
	
	print "Number of polygons found: %d\n" % (mgGetRecListCount (list))
	
	if mgGetRecListCount (list) == 10:
		print "find test succeeded"
	else:
