...
def RedoFunc (db, undoData):
	...
	# for redo, have to set the mask after the UVs
...
def UndoFunc (db, undoData):
	...
	for j in undoData.undoFaceList:
		j.swap(undoData.textureCode)
	# for undo, have to set the mask before the UVs
	for l in undoData.undoMeshVtxMaskList:
		l.swap(undoData.layer)
	for k in undoData.undoMeshVtxList:
		k.swap(undoData.layer)
...
class deleteLayerData():
	def __init__(self, db, layer):
		self.layer = layer
		self.undoData = undoData (layer)
		self.textureCode = self.undoData.textureCode
		self.uCode = self.undoData.uCode
		self.vCode = self.undoData.vCode
	def appendUndo(self, faceOrMesh):
		undoFace = undoFaceTextureIndex (faceOrMesh, self.textureCode)
		...
		if (mgGetCode(faceOrMesh) == fltPolygon):
			...
			for i in range(0,numVtx):
				undoMeshVtx = undoMeshVtxUV(faceOrMesh, i, self.layer)
				self.undoData.undoMeshVtxList.append(undoMeshVtx)
			
	def applyToFace(self, face):
		self.appendUndo(face)
		mgSetAttList (face, self.textureCode, -1)
		vtx = mgGetChild (face)
		while (vtx):
			mgSetAttList (vtx, self.uCode, 0.0, self.vCode, 0.0)
			vtx = mgGetNext (vtx)		
	def applyToMesh(self, mesh):
		self.appendUndo(mesh)
		mgSetAttList (mesh, self.textureCode, -1)
		maskBit = GetMeshUVMaskBit (self.layer)
		...
		for i in range(0,numVtx):
			mgMeshSetVtxUV (mesh, i, self.layer, 0.0, 0.0)

def DeleteTextureLayerOnFace(face, data):
	data.applyToFace(face)

def DeleteTextureLayerOnMesh(mesh, data):
	data.applyToMesh(mesh)
	
def CheckPolygonOrMesh (db, parent, rec, data):
	code = mgGetCode (rec)
	if (code == fltPolygon):
		DeleteTextureLayerOnFace (rec, data)
	elif (code == fltMesh):
		DeleteTextureLayerOnMesh (rec, data)	
	return MG_TRUE

def DeleteTextureLayer(layer):
	toolName = "Delete Texture Layer"
	...
	if (layer == -1):
		# layer is "blend", cannot delete this layer
		mgSendMessage (MMSG_ERROR, "Cannot Delete Texture Layer, invalid layer specified")
		return
		
	data = deleteLayerData (db, layer)
...
# change this to control which texture layer is deleted:
db = mgGetCurrentDb()
b, layer = mgGetCurrentTextureLayer(db)
DeleteTextureLayer (layer)
