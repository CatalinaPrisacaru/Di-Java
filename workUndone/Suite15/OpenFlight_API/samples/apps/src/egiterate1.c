...
static mgbool ColorRange ( mgrec* db, mgrec* parent, mgrec* rec, void* userData )
{
	mgbool result = MG_FALSE;
	int check;
	unsigned int color;
	if (mgGetCode ( rec ) == fltPolygon)
	{
		check = mgGetAttList (rec, fltPolyPrimeColor, &color, MG_NULL);
		if (check)
		{
			if (color >= 0 && color < 10)
			{
...
static mgbool WalkFromThere (mgrec* db, mgrec* parent, mgrec* rec, void* userData)
{
	int code;
	mgSendMessage (MMSG_STATUS, "WalkFromThere called");
	code = mgGetCode (rec);
	map[code] = map[code]+1;
	return MG_TRUE;
}

static mgbool PrintNodeByCode (mgrec* db, mgrec* parent, mgrec* rec, void* userData)
{
	mgSendMessage (MMSG_STATUS, "PrintNodeByCode called");
	mgWalk (rec, MG_NULL, WalkFromThere, map, MWALK_ON);
	return MG_TRUE;
}
...
int main (int argc, char* argv[])
{
   ...
   printf ("\nCreating database: %s\n", argv[1]);
	...
	list = mgFind (db, ColorRange, MG_NULL, MWALK_ON);
	
	printf ("Number of polygons found: %d\n", mgGetRecListCount (list));
	
	if (mgGetRecListCount (list) == 10)
		printf ("expression test succeeded\n");
	else
		printf ("expression test failed\n");

	map[fltPolygon] = 0;
	mgIterateRecList (list, PrintNodeByCode, map);

	printf ("(%d:%d)\n", fltPolygon, map[fltPolygon]);
